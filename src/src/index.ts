...
function extractTrackInfo(playlists: Array<Array<any>>): Array<Array<TrackType>> {
...
function createPlaylist() {
  let newPlaylist =  spotifyAPI.createPlaylist(
    `New merged playlist ${uuid4().substring(0, 10)}`,
    {'description': 'Thank you for using Setify, we appreciate your support.', 'public': true}
    ).then((data: any) => {
      console.log(data);
      return {id: data.body.id, name: data.body.name, url: data.body.href} as NewListType;;
    }).catch((err: any) => {
      console.log(err);
...
function insertTracks(playlistID: string, tracks: Array<TrackType>) {
  ...
  for(let i=0; i < Math.ceil(tracks.length/100); i++) {
    ...
    let uris = tracks.slice(i*100, endSlice).map((track: TrackType) => {return track.uri});
    spotifyAPI.addTracksToPlaylist(playlistID, uris).then((data: any) => {
    }).catch((err: any) => {
      result = false;
      console.log(err);
    });
...
app.get('/merge-playlists', (req: any, res: any) => {
  ...
  requestTracks(data['playlists']).then((tracks: any) => {
    ...
    const combinedLists = combineLists(data['method'], trackInfo) as Array<TrackType>;
    createPlaylist().then((newPlaylist: NewListType) => {
