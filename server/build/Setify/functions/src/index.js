...
const spotifyCredentials = {
    ...
    redirectURI: 'https://us-central1-setify-b8238.cloudfunctions.net/app/callback'
};
...
function generateRandomString(length) {
    ...
    for (let i = 0; i < length; i++) {
        random = random + String.fromCharCode(Math.floor(Math.random() * 25) + 65);
    }
    return random;
}
app.get('/login', (req, res) => {
    ...
    res.redirect(spotifyAPI.createAuthorizeURL(scopes, state) + '&show_dialog=true' + '&client_id=' + process.env.SPOTIFY_ID + '&redirect_uri=https://us-central1-setify-b8238.cloudfunctions.net/app/callback'); //Redirects user to spotify login to provide specified permissions
    // const url = 'https://accounts.spotify.com/authorize?' +
    // querystring.stringify({
    //   response_type: 'code',
    //   client_id: process.env.SPOTIFY_ID,
    //   scope: scopes,
    //   redirect_uri: 'https://us-central1-setify-b8238.cloudfunctions.net/app/callback',
    //   state: state
    // });
    // axios({
    //   method: 'GET',
...
app.get('/get-playlists', (req, res) => {
    spotifyAPI.getUserPlaylists(req.headers['user']).then((data) => {
...
function createPlaylist() {
    let newPlaylist = spotifyAPI.createPlaylist(`New merged playlist ${(0, uuid_1.v4)().substring(0, 10)}`, { 'description': 'Thank you for using Setify, we appreciate your support.', 'public': true }).then((data) => {
        console.log(data);
        return { id: data.body.id, name: data.body.name, url: data.body.href };
        ;
    }).catch((err) => {
        console.log(err);
...
function insertTracks(playlistID, tracks) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let i = 0; i < Math.ceil(tracks.length / 100); i++) {
            ...
            let uris = tracks.slice(i * 100, endSlice).map((track) => { return track.uri; });
            result = spotifyAPI.addTracksToPlaylist(playlistID, uris).then((data) => {
                return true;
            }).catch((err) => {
                return false;
            });
